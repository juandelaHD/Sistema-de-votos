...
const (
	PRESIDENTE TipoVoto = iota // Presidente TipoVoto = 0
	GOBERNADOR                 // Gobernador TipoVoto = 1
	INTENDENTE                 // Intendente TipoVoto = 2
)
...
type Voto struct {
	sobre     [CANT_VOTACION]int //  [ PRENSIDENTE, GOBERNADOR, INTENDENTE ]
	impugnado bool
}
...
type Votante interface {
	...
	Votar(tipovoto TipoVoto, alternativa int) error
	...
	FinVoto() ([CANT_VOTACION]int, bool, error)

	//FinalizoVoto nos lee si el votante ya voto una vez
	FinalizoVoto() bool
}
